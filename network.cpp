//Changes to program were made by Justin Henn, Zach Jorn, and Paul Ries. We still used a lot of the original code made by Dr. Climer. We implemented colors to nodes after thethe nodes have been sorted. We start with the sorting with the node with the most connections.
/***********************************************************
/
...
double Vertex::getWeight(int endpoint) // return weight of edge
{
  Edge *edgePtr; // pointer to move through linked list of edges
  edgePtr = &firstEdge; // point to first edge
  while (edgePtr->target != endpoint && edgePtr->next != 0){ // follow until find last edge
    //prev = edgePtr;
    edgePtr = edgePtr->next; // pointer points at next edge in list
}
...
int Network::removeEdge(int v1, int v2) // return 1 if successfully add edge
{
...
void Network::printEdges(char *outputFile, int arr[]) // print all edges in network
{
  FILE *output;
  char colors[5][30] = {"Black", "Green", "Blue", "Red", "Yellow"}; //Colors for what the nodes are to be colored. The black will be the color of the singletons.
  ...
  for (int i = 0; i < numVertices; i++) { //goes through the nodes and colors them accordingly
...
void Network::bfs(char *outputFile, int arr[]) 
{
  FILE *output;
  ...
  for (int i = 0; i < numVertices; i++) {
      long int numberEdges = 0;
      int degree = getDegree(i);
      for(int j = 0; j < ptr; j++) {
		numberEdges += getDegree(component[j]);
		if (getDegree(component[j]) > getDegree(i)) {
			degree = (getDegree(component[j]));
		}
      }			
